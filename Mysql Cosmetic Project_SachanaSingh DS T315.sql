...
select ProductPrice,ProductName from Product where ProductPrice in (select max(ProductPrice) from Product);
...
select ProductPrice, ProductName from Product where ProductPrice in (select min(ProductPrice) from Product);

/*4.Display ProductId, Name, Price and Manufacturing date of  Product whos Manufacturing date is after 2020-08-09 12:00:00.*/
 select ProductId, ProductName, ProductPrice,ManufacturingDate  from Product where ManufacturingDate > '2020-08-09 12:00:00';
...
select max(ProductPrice) as MaxProductPrice, ProductName from Product group by ProductName having max(ProductPrice)>1000;

/*6.Display names, Price, ManufacturingDate of the Products whose name starts with 'C' in descending order of Price.*/
select ProductName, ProductPrice, ManufacturingDate from Product where ProductName like 'C%' order by ProductPrice desc;

/*7.Display ProductId, ProdName, productPrice, validity  period of  all the products.*/
select ProductId, ProductName, ProductPrice, datediff(Usebeforedate,ManufacturingDate) as ValidforDays from Product;
...
Select ProductId,ProductName, Usebeforedate From Product where ProductPrice = ANY (Select ProductPrice from Product where ProductPrice in (499,1300,2549));
...
select ProductName,ProductPrice,ManufacturingDate, UsebeforeDate from Product where ProductPrice > (select ProductPrice from product where ProductId = 23);
...
select ProductName, ProductPrice, ManufacturingDate from Product where ProductPrice > (select ProductPrice from Product where ProductId = 19);
...
/* JOINS:
 ...
 /*2.Display Brand Name, Product Name of only the Products named Foundation, Concealer, Eyeliner with date of Manufacturing performing the suitable join.*/
  select BrandName, ProductName, ManufacturingDate 
  from Brand inner join Product
  on Product.ProductId = Brand.BrandId 
  where ProductName in ('Foundation', 'Concealer',' Mascara','Eyeliner');
 ...
 select CountryOfOrigin , ProductName, ProductPrice 
